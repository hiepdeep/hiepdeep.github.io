			console.clear();
			console.log("Gerber Viewer Engine - Mã nguồn xây dựng vào 17/09/2026 bởi HiepDz");
			...
			let boxStartWorld = null, boxEndWorld = null;
			...
			// PARSER GERBER (RS-274X PARSER ENGINE)
			class GerberParser {
				...
				static parseMacroBody(body, unitScale) {
					...
					for (let line of lines) {
						...
						if (code === 1 && tokens.length >= 5) {
							...
							let rot = 0, endIdx = tokens.length;
			...
			document.addEventListener("DOMContentLoaded", () => {
				...
				// CHUYỂN ĐỔI TOOL & ĐIỀU KHIỂN TRẠNG THÁI NÚT
				...
				function drawCenterMarker(x, y, labelColor = "rgba(38, 127, 217, 1)") {
				...
				function drawOverlay() {
					...
					if (boxStartWorld && boxEndWorld && (isBoxSelecting || activeTool === "SELECT_AREA")) {
						...
						ctx.strokeStyle = "rgba(38, 127, 217, 0.5)";
						ctx.lineWidth = 1.5;
						ctx.setLineDash([5, 4]);
						ctx.fillStyle = "rgba(38, 127, 217, 0.25)";
						ctx.fillRect(bx, by, bw, bh);
						ctx.strokeRect(bx, by, bw, bh);
						ctx.setLineDash([]);
					}
					if (activeTool === "SELECT_POINT" && pinnedPoint) {
						drawCenterMarker(pinnedPoint.x, pinnedPoint.y, "rgba(38, 127, 217, 1)");
						...
						drawCenterMarker(cx, cy, "rgba(38, 127, 217, 1)");
					}
					if (activeTool === "MEASURE" && measureStart) {
						...
						ctx.setLineDash([]);
						[p1, p2].forEach(p => {
							ctx.fillStyle = "rgba(38, 127, 217, 1)";
							ctx.beginPath();
						...
						drawCenterMarker(midX, midY, "rgba(38, 127, 217, 1)");
					}
				}
